        ...
        public static void TERMIPSterms(int NI, int NJ, int NK)
        {
            Parallel.For(2, NI, Program.pOptions, i =>
            {
                ...
                for (int j = 2; j <= NJ - 1; j++)
                {
                    ...
                    double[] VISHiP = Program.VISH[i + 1][j];
                    double[] VISHiM = Program.VISH[i - 1][j];
                    double[] VISHjP = Program.VISH[i][j + 1];
                    double[] VISHjM = Program.VISH[i][j - 1];
                    float[] AREAXiP = Program.AREAX[i + 1][j];
                    float[] AREAXjP = Program.AREAY[i][j + 1];
                    double[] UNiP = Program.UN[i + 1][j];
                    double[] UNiM = Program.UN[i - 1][j];
                    double[] VNjP = Program.VN[i][j + 1];
                    double[] VNjM = Program.VN[i][j - 1];
                    float[] RhoiP = Program.RHO[i + 1][j];
                    float[] RhoiM = Program.RHO[i - 1][j];
                    float[] RhojP = Program.RHO[i][j + 1];
                    float[] RhojM = Program.RHO[i][j - 1];
