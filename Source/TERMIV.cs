...
namespace GRAMM_CSharp_Test
{
	partial class Program
	{
		...
		public static void TERMIVterms(int NI, int NJ, int NK)
		{
			...
			Parallel.ForEach(Partitioner.Create(1, NI+1, range_parallel), range =>
			{
				...
				float AEREA_LL = 0, AEREAZX_LL = 0, AEREAZY_LL = 0,  AREA_T1 = 0,  AREA_T2 = 0,  AREA_T3 = 0,  AREA_T4 = 0;
				double DDX_Rez = 0.0001F, DDY_Rez = 0.0001F;
				float RHO = 0,  RHO_KP = 0,  RHO_L_KM1 = 0, RHOKM, RHOKP;
				...
				for (int i = range.Item1; i < range.Item2; i++)
				{
					...
					for (int j = 1; j <= NJ_P; j++)
					{
						...
						{
							DDX_Rez = 1 / Program.DDX[i]; DDY_Rez = 1 / Program.DDY[j];
						}
						...
						for (int kn = 1; kn <= 2 * NK_P; kn++)
						{
							...
							if (kn % 2 != 0) // ODD numbers 1,3,5,..- new k value - set local constants new
							{
								...
								if ((i > 1) && (j > 1) && (i < NI_P) && (j < NJ_P) && (k < NK_P))
								{
									...
									RHOKP = 0.5F * (RHO_KP + RHO);
									VISKP = 0.5F * (VISV_L[k + 1] + VISV_L[k]);
									...
									if (k > 1)
									{
										RHOKM = 0.5F * (RHO_L_KM1 + RHO);
										VISKM = 0.5F * (VISV_L[k - 1] + VISV_L[k]);
							...
							if ((m - 1) == 1)
							{
								...
								if (i > 1) DW1 = 4 * VISH * VISH_IM * VISH_T1 *
									AREAX_L[k] * DDX_Rez * 0.5F * (RHO + Program.RHO[i - 1][j][k]);
								DS = 0;
								if (j > 1) DS = 4 * VISH * VISH_JM * VISH_T2 *
									AREAY_L[k] * DDY_Rez * 0.5F * (RHO + Program.RHO[i][j - 1][k]);

								//Advection terms
								FE = (0.5F * RHO * (U1_L[k] + U2_L[k]) * AREA_T1 +
								      0.5F * RHO * (V1_L[k] + V2_L[k]) * AREA_T2 +
								      0.5F * RHO * (W1_L[k] + W2_L[k]) * AEREA_LL);

								FB = 0;
								if (k > 1)
									FB = (0.5F * (W1_L[k] * RHO + W2_L[k - 1] * RHO_L_KM1) * AEREA_LL +
									      0.5F * (U1_L[k] * RHO + U2_L[k - 1] * RHO_L_KM1) * AREAZX_L[k] +
									      0.5F * (V1_L[k] * RHO + V2_L[k - 1] * RHO_L_KM1) * AREAZY_L[k]);

								if (i > 1)
									FW1 = 0.5F * (U1_L[k] * RHO + Program.U2[i - 1][j][k] * Program.RHO[i - 1][j][k]) * AREAX_L[k];
								else
									FW1 = U1_L[k] * RHO * AREAX_L[k];

								if (j > 1)
									FS = 0.5F * (V1_L[k] * RHO + V2_L_M * Program.RHO[i][j - 1][k]) * AREAY_L[k];
								else
                            ...
                            Program.SU[i][j][kn] = CPI;
								...
								BIM_L[kn] = (float) (DE * Math.Max(0, Pow5(1 - 0.1F * PE)) + Math.Max(-FE, 0));
								CIM_L[kn] = (float) (DB * Math.Max(0, Pow5(1 - 0.1F * PB)) + Math.Max(FB, 0));
								AW1_L[k]  = (float) (DW1 * Math.Max(0, Pow5(1 - 0.1F * PW1)) + Math.Max(FW1, 0));
								AS1_L[k]  = (float) (DS * Math.Max(0, Pow5(1 - 0.1F * PS)) + Math.Max(FS, 0));

								AP0_L[k] = (float) (VOL_L[k] * 0.5F / Program.DT * RHO);
								...
								if ((i > 1) && (j > 1) && (i < NI_P) && (j < NJ_P) && (k < NK_P) && (k > 1))
								{
									...
									double DUDXB = 2 * 0.5F * ((U2_L[k] - U1_L[k]) + (U2_L[k - 1] - U1_L[k - 1])) * DDX_Rez;
									double DUDYB = 2 * 0.5F * ((U2_L[k] - U1_L[k]) + (U2_L[k - 1] - U1_L[k - 1])) * DDY_Rez;

									double DVDXS = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (V2_L_M - Program.V1[i][j - 1][k])) * DDX_Rez;
									double DVDXE = 2 * (V2_L[k] - V1_L[k]) * DDX_Rez;
									double DVDXB = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (V2_L[k - 1] - V1_L[k - 1])) * DDX_Rez;

									double DWDXE = 2 * (W2_L[k] - W1_L[k]) * DDX_Rez;
									double DWDXB = 2 * 0.5F * ((W2_L[k] - W1_L[k]) + (W2_L[k - 1] - W1_L[k - 1])) * DDX_Rez;
									...
									double DVDYS = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (V2_L_M - Program.V1[i][j - 1][k])) * DDY_Rez;
									double DVDYE = 2 * (V2_L[k] - V1_L[k]) * DDY_Rez;
									double DVDYB = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (V2_L[k - 1] - V1_L[k - 1])) * DDY_Rez;
									
									double DUDYW = 2 * 0.5F * ((U2_L[k] - U1_L[k]) + (Program.U2[i - 1][j][k] - Program.U1[i - 1][j][k])) * DDY_Rez;
									double DUDYE = 2 * (U2_L[k] - U1_L[k]) * DDY_Rez;

									double DWDYE = 2 * (W2_L[k] - W1_L[k]) * DDY_Rez;
									double DWDYB = 2 * 0.5F * ((W2_L[k] - W1_L[k]) + (W2_L[k - 1] - W1_L[k - 1])) * DDY_Rez;
									...
									double DUDZW = 0.5F * ((U1_L[k] - Program.U2[i - 1][j][k]) + (U2_L[k] - U1_L[k]))  * f1;
									double DUDZB = (U1_L[k] - U2_L[k - 1])  * f2;

									double DVDZE = (V2_L[k] - V1_L[k])  * f1;
									double DVDZS = 0.5F * ((V1_L[k] - V2_L_M) + (V2_L[k] - V1_L[k]))  * f1;
							...
							else
							{
								...
								if (i < NI_P) DE2 = 4 * VISH * VISH_I * VISH_R2 *
									AREA_T3 * DDX_Rez * 0.5F * (RHO + Program.RHO[i + 1][j][k]);
								DS2 = 0;
								DS2 = 2 * VISH * (AREA_T1 * DDX_Rez + AREA_T2 * DDY_Rez) * RHO;

								DN2 = 0;
								if (j < NJ_P) DN2 = 4 * VISH * VISH_J * VISH_R1 *
									AREA_T4 * DDY_Rez * 0.5F * (RHO + Program.RHO[i][j + 1][k]);
								DT2 = 0;
								...
								//Advection terms
								if (i < NI_P)
									FE2 = 0.5F * (U2_L[k] * RHO + Program.U1[i + 1][j][k] * Program.RHO[i + 1][j][k]) * AREA_T3;
								else
									FE2 = U2_L[k] * RHO * AREA_T3;

								FS2 = (0.5F * RHO * (U1_L[k] + U2_L[k]) * AREA_T1 +
								       0.5F * RHO * (V1_L[k] + V2_L[k]) * AREA_T2 +
								       0.5F * RHO * (W1_L[k] + W2_L[k]) * AEREA_LL);

								if (j < NJ_P)
									FN2 = 0.5F * (V2_L[k] * RHO + Program.V1[i][j + 1][k] * Program.RHO[i][j + 1][k]) * AREA_T4;
								else
									FN2 = V2_L[k] * RHO * AREA_T4;

								FT2 = 0;
								if (k < NK_P)
									FT2 = (0.5F * (W2_L[k] * RHO + W1_L[k + 1] * RHO_KP) * AEREA_LL +
									       0.5F * (U2_L[k] * RHO + U1_L[k + 1] * RHO_KP) * AEREAZX_LL +
									       0.5F * (V2_L[k] * RHO + V1_L[k + 1] * RHO_KP) * AEREAZY_LL);
								else
								...
								//Peclet numbers
								DE2 = Math.Max(DE2, 0.0001F);
								DT2 = Math.Max(DT2, 0.0001F);
								DN2 = Math.Max(DN2, 0.0001F);
								DS2 = Math.Max(DS2, 0.0001F);
                            ...
                            Program.SU[i][j][kn] = CPI;
								...
								//advection scheme "power-law" by PataNK_Par 1980, p90
								BIM_L[kn] = (float) (DT2 * Math.Max(0, Pow5(1 - 0.1F * PT2)) + Math.Max(-FT2, 0));
								CIM_L[kn] = (float) (DS2 * Math.Max(0, Pow5(1 - 0.1F * PS2I)) + Math.Max(FS2, 0));
								AE2_L[k]  = (float) (DE2 * Math.Max(0, Pow5(1 - 0.1F * PE2)) + Math.Max(-FE2, 0));
								AN2_L[k]  = (float) (DN2 * Math.Max(0, Pow5(1 - 0.1F * PN2)) + Math.Max(-FN2, 0));
								...
								if ((i > 1) && (j > 1) && (i < NI_P) && (j < NJ_P) && (k < NK_P) && (k > 1))
								{
									...
									double DUDXT = 2 * 0.5F * ((U2_L[k] - U1_L[k]) + (U2_L[k + 1] - U1_L[k + 1])) * DDX_Rez;
									double DUDYT = 2 * 0.5F * ((U2_L[k] - U1_L[k]) + (U2_L[k + 1] - U1_L[k + 1])) * DDY_Rez;

									double DVDXN = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (Program.V2[i][j + 1][k] - Program.V1[i][j + 1][k])) * DDX_Rez;
									double DVDXS = 2 * (V2_L[k] - V1_L[k]) * DDX_Rez;
									double DVDXT = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (V2_L[k + 1] - V1_L[k + 1])) * DDX_Rez;

									double DWDXS = 2 * (W2_L[k] - W1_L[k]) * DDX_Rez;
									double DWDXT = 2 * 0.5F * ((W2_L[k] - W1_L[k]) + (W2_L[k + 1] - W1_L[k + 1])) * DDX_Rez;
									...
									double DVDYN = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (Program.V2[i][j + 1][k] - Program.V1[i][j + 1][k])) * DDY_Rez;
									double DVDYW = 2 * (V2_L[k] - V1_L[k]) * DDY_Rez;
									double DVDYT = 2 * 0.5F * ((V2_L[k] - V1_L[k]) + (V2_L[k + 1] - V1_L[k + 1])) * DDY_Rez;

									double DUDYE = 2 * 0.5F * ((U2_L[k] - U1_L[k]) + (Program.U2[i + 1][j][k] - Program.U1[i + 1][j][k])) * DDY_Rez;
									double DUDYW = 2 * (U2_L[k] - U1_L[k]) * DDY_Rez;

									double DWDYW = 2 * (W2_L[k] - W1_L[k]) * DDY_Rez;
									double DWDYT = 2 * 0.5F * ((W2_L[k] - W1_L[k]) + (W2_L[k + 1] - W1_L[k + 1])) * DDY_Rez;
									...
									double DUDZE = 0.5F * ((Program.U1[i + 1][j][k] - U2_L[k]) + (U2_L[k] - U1_L[k]))  * f1;
									double DUDZT = (U1_L[k + 1] - U2_L[k])  * f1;

									double DVDZW = (V2_L[k] - V1_L[k])  * f1;
									double DVDZN = 0.5F * ((Program.V1[i][j + 1][k] - V2_L[k]) + (V2_L[k] - V1_L[k]))  * f1;
