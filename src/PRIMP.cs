                ...
                Parallel.ForEach(Partitioner.Create(2, NI, range_parallel), range =>
                {
                    ...
                    for (int i = range.Item1; i < range.Item2; ++i)
                    {
                        ...
                        double[] AIMj_L   = Program.AIM[i][jP1];
                            ...
                            ReadOnlySpan<float> AREAYj_L = Program.AREAY[i][jP1];
                            ReadOnlySpan<float> AREAZ_L = Program.AREAZ[i][j];
                            ReadOnlySpan<float> AREAXYZ_L = Program.AREAXYZ[i][j];
                            ReadOnlySpan<float> AREAZX_L = Program.AREAZX[i][j];
                            ReadOnlySpan<float> AREAZXi_L = Program.AREAZX[iP1][j];
                            ReadOnlySpan<float> AREAZY_L = Program.AREAZY[i][j];
                            ReadOnlySpan<float> AREAZYj_L = Program.AREAZY[i][jP1];
                            ReadOnlySpan<float> RHO_L = Program.RHO[i][j];
                            ReadOnlySpan<float> RHOi_L = Program.RHO[iP1][j];
                            ReadOnlySpan<float> RHOj_L = Program.RHO[i][jP1];
                ...
                Parallel.ForEach(Partitioner.Create(2, NI, range_parallel), range =>
                {
                    ...
                    for (int i = range.Item2 - 1; i >= range.Item1; --i)
                    {
                        ...
                        double[] AIMmj_L  = Program.AIM[i][j - 1];
                            ...
                            ReadOnlySpan<float> AREAY_L = Program.AREAY[i][j];
                            ReadOnlySpan<float> AREAYmj_L = Program.AREAY[i][j - 1];
                            ReadOnlySpan<float> AREAZ_L = Program.AREAZ[i][j];
                            ReadOnlySpan<float> AREAXYZ_L = Program.AREAXYZ[i][j];
                            ReadOnlySpan<float> AREAZX_L = Program.AREAZX[i][j];
                            ReadOnlySpan<float> AREAZXi_L = Program.AREAZX[i - 1][j];
                            ReadOnlySpan<float> AREAZY_L = Program.AREAZY[i][j];
                            ReadOnlySpan<float> AREAZYmj_L = Program.AREAZY[i][j - 1];
                            ReadOnlySpan<float> RHO_L = Program.RHO[i][j];
                            ReadOnlySpan<float> RHOmi_L = Program.RHO[i - 1][j];
                            ReadOnlySpan<float> RHOmj_L = Program.RHO[i][j - 1];
                            ReadOnlySpan<float> SUXYZ_L = Program.SUXYZ[i][j];
                ...
                Parallel.ForEach(Partitioner.Create(2, NJ, range_parallel), range =>
                {
                    ...
                    for (int j = range.Item1; j < range.Item2; ++j)
                    {
                        ...
                        double[] AIMj_L   = Program.AIM[i][jP1];
                            ...
                            ReadOnlySpan<double> DPZj_L = Program.DPZ[i][jP1];
                            ReadOnlySpan<float> AREAX_L = Program.AREAXImm[i][j];
                            ReadOnlySpan<float> AREAXi_L = Program.AREAX[iP1][j];
                            ReadOnlySpan<float> AREAY_L = Program.AREAY[i][j];
                            ReadOnlySpan<float> AREAYj_L = Program.AREAY[i][jP1];
                            ReadOnlySpan<float> AREAZ_L = Program.AREAZ[i][j];
                            ReadOnlySpan<float> AREAXYZ_L = Program.AREAXYZ[i][j];
