                        ...
                        for (int k = 2; k <= Program.NZB - 1; k++)
                        {
                            ...
                            else if (k == 2)
                            {
                                ...
                                */
                                ...
                                double Wqu = Program.XWQ[i][j] * Program.RHO[i][j][1] * Program.UST[i][j] * ALW1 * (Program.QU[i][j][1] - Program.QUG[i][j]) * 0.001;

                                //amount of water evaporated from the soil
                                Program.QU[i][j][1] += -Wqu / ALW1 * 1000.0 * Program.DT * Program.AREAImm[i][j][1] / Program.VOLImm[i][j][1] / Program.RHO[i][j][1];

                                Wqu = Math.Min(Wqu, 20);
                                Wqu = Math.Max(Wqu, -400);
                                EINTRAG4 += Wqu;

                                TERMTB = (DTBDZ + (Program.WQU[i][j] / Program.DDXImm[i] / Program.DDYImm[j] + Wqu + Program.RSOLG[i][j] +
                                        (Program.RL[i][j] - Program.EPSG[i][j] * Program.SIGMA * Pow4(Program.TB[i][j][2]))
                                        + Program.AWQ[i][j]) / Program.DZB[2]) * Program.DT;
                                Eintrag5Local += DTBDZ * Program.DZB[2];
